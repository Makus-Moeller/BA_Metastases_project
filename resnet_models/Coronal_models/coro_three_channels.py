...
#TRAIN
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/coro/three*'): 
  scan = np.load(axi_scan_anony)
  flipped_scan_horizontal = np.fliplr(scan).copy()
  flipped_scan_vertical = np.flipud(scan).copy()
  rescaled = scan * 1.1
  X_train.append(flipped_scan_horizontal)
  X_train.append(flipped_scan_vertical)
  X_train.append(rescaled)
  X_train.append(scan)
  y_train.append(1.0)
  y_train.append(1.0)
  y_train.append(1.0)
...
for axi_scan_control in data_path.glob('nii/controls/train/*/coro/three*'): 
...
#VALIDATION
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/coro/three*'):
    X_valid.append(np.load(axi_scan_anony))
    y_valid.append(1.0)

for axi_scan_control in data_path.glob('nii/controls/val/*/coro/three*'):
    X_valid.append(np.load(axi_scan_control))
    y_valid.append(0.0)
...
#TEST
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/coro/three*'):
    X_test.append(np.load(axi_scan_anony))
    y_test.append(1.0)

for axi_scan_control in data_path.glob('nii/controls/test/*/coro/three*'):
    X_test.append(np.load(axi_scan_control))
    y_test.append(0.0)
...
transform = transforms.Compose([transforms.Resize(512)]) # is an option because Imagenet is 256x256
...
for i in range(epochs):
    ...
    is_best = tst_corr*100/((b+1)*batch_size) > best_prec1
    best_prec1 = max(tst_corr*100/((b+1)*batch_size), best_prec1)
    save_checkpoint({
            'epoch': i + 1,
