...
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/coro/PET*'): 
...
for axi_scan_control in data_path.glob('nii/controls/train/*/coro/PET*'): 
...
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/coro/PET*'): 
  scan = np.load(axi_scan_anony)
  X_valid.append(expand_np_image_to_3_channels(scan))
  y_valid.append(1.0)

for axi_scan_control in data_path.glob('nii/controls/val/*/coro/PET*'):
  scan = np.load(axi_scan_control)
  X_valid.append(expand_np_image_to_3_channels(scan))
  y_valid.append(0.0)
...
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/coro/PET*'):
  scan = np.load(axi_scan_anony)
  X_test.append(expand_np_image_to_3_channels(scan))
  y_test.append(1.0)

for axi_scan_control in data_path.glob('nii/controls/test/*/coro/PET*'):
  scan = np.load(axi_scan_control)
  X_test.append(expand_np_image_to_3_channels(scan))
  y_test.append(0.0)
...
transform = transforms.Compose([transforms.Resize(512)]) # Follow same resolution as github repo
