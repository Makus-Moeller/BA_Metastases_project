...
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/coro/norm*'): 
...
for axi_scan_control in data_path.glob('nii/controls/train/*/coro/norm*'): 
...
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/coro/norm*'):
  scan = np.load(axi_scan_anony)
...
for axi_scan_control in data_path.glob('nii/controls/val/*/coro/norm*'):
  scan = np.load(axi_scan_control)
...
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/coro/norm*'):
  scan = np.load(axi_scan_anony)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8) #If we want to squezze values between 0-1 
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for axi_scan_control in data_path.glob('nii/controls/test/*/coro/norm*'):
  scan = np.load(axi_scan_control)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8)
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
