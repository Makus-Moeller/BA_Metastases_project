...
for coro_scan_anony in data_path.glob('nii/anonymized/val/*/coro/PET*'): 
  scan = np.load(coro_scan_anony)
  Xt_pet.append(expand_np_image_to_3_channels(scan))
  yt_pet.append(1.0)

for coro_scan_control in data_path.glob('nii/controls/val/*/coro/PET*'): 
...
for coro_scan_anony in data_path.glob('nii/anonymized/val/*/coro/CT_*'): 
  scan = np.load(coro_scan_anony)
  Xt_ct.append(expand_np_image_to_3_channels(scan))
  yt_ct.append(1.0)

for coro_scan_control in data_path.glob('nii/controls/val/*/coro/CT_*'): 
  scan = np.load(coro_scan_control)
  Xt_ct.append(expand_np_image_to_3_channels(scan))
  yt_ct.append(0.0)
...
for coro_scan_anony in data_path.glob('nii/anonymized/val/*/coro/norm_*'): 
  scan = np.load(coro_scan_anony)
  Xt_norm.append(expand_np_image_to_3_channels(scan))
  yt_norm.append(1.0)

for coro_scan_control in data_path.glob('nii/controls/val/*/coro/norm_*'): 
  scan = np.load(coro_scan_control)
  Xt_norm.append(expand_np_image_to_3_channels(scan))
  yt_norm.append(0.0)
...
for coro_scan_anony in data_path.glob('nii/anonymized/val/*/coro/three_*'): 
  scan = np.load(coro_scan_anony)
  Xt_three.append(scan)
  yt_three.append(1.0)

for coro_scan_control in data_path.glob('nii/controls/val/*/coro/three_*'): 
  scan = np.load(coro_scan_control)
  Xt_three.append(scan)
  yt_three.append(0.0)
