...
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/axi/norm*'): 
...
for axi_scan_control in data_path.glob('nii/controls/train/*/axi/norm*'): 
...
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/axi/norm*'):
  scan = np.load(axi_scan_anony)
...
for axi_scan_control in data_path.glob('nii/controls/val/*/axi/norm*'):
  scan = np.load(axi_scan_control)
...
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/axi/norm*'):
  scan = np.load(axi_scan_anony)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8) #If we want to squezze values between 0-1 
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for axi_scan_control in data_path.glob('nii/controls/test/*/axi/norm*'):
  scan = np.load(axi_scan_control)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8)
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
