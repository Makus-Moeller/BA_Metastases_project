...
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/sagi/norm*'): 
...
for axi_scan_control in data_path.glob('nii/controls/train/*/sagi/norm*'): 
...
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/sagi/norm*'):
  scan = np.load(axi_scan_anony)
...
for axi_scan_control in data_path.glob('nii/controls/val/*/sagi/norm*'):
  scan = np.load(axi_scan_control)
...
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/sagi/norm*'):
  scan = np.load(axi_scan_anony)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8) #If we want to squezze values between 0-1 
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for axi_scan_control in data_path.glob('nii/controls/test/*/sagi/norm*'):
  scan = np.load(axi_scan_control)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8)
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for i in range(epochs):
    ...
    is_best = tst_corr > best_prec1
    best_prec1 = max(tst_corr, best_prec1)
    save_checkpoint({
            'epoch': i + 1,
