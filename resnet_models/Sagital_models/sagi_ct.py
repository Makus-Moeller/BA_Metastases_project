...
#TRAIN
for axi_scan_anony in data_path.glob('nii/anonymized/train/*/sagi/CT_*'): 
...
for axi_scan_control in data_path.glob('nii/controls/train/*/sagi/CT_*'): 
...
#VALIDATION
for axi_scan_anony in data_path.glob('nii/anonymized/val/*/sagi/CT_*'):
  scan = np.load(axi_scan_anony)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8) #If we want to squezze values between 0-1
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for axi_scan_control in data_path.glob('nii/controls/val/*/sagi/CT_*'):
  scan = np.load(axi_scan_control)
...
#TEST
for axi_scan_anony in data_path.glob('nii/anonymized/test/*/sagi/CT_*'):
  scan = np.load(axi_scan_anony)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8) #If we want to squezze values between 0-1 
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
for axi_scan_control in data_path.glob('nii/controls/test/*/sagi/CT_*'):
  scan = np.load(axi_scan_control)
  #rescaled_scan = ((scan / np.amax(scan))*255).astype(np.uint8)
  #amax = np.amax(scan)
  #rescaled_scan = scan / amax if amax > 0.0 else scan
...
def save_checkpoint(state, is_best, filename=f'{model_name}_ch.pth.tar'):
    if is_best:
