...
controls = []
anonymized = []
for pt in anonymized_path.glob("Cmet*/"):
    anonymized.append(pt)
for pt in control_path.glob("Cmet*/"):
    controls.append(pt)
...
X_train_control, X_test_control, _, _ = train_test_split(controls, np.zeros(len(controls)), test_size=0.15, random_state=seed)
X_train_anonymized, X_test_anonymized, _, _ = train_test_split(anonymized, np.zeros(len(anonymized)), test_size=0.15, random_state=seed)

for i in X_train_control:
    print(i)
    os.system(f'mv {i} {control_path}/train')


for i in X_train_anonymized:
    os.system(f'mv {i} {anonymized_path}/train')

# In[39]:


for i in X_test_anonymized:
    os.system(f'mv {i} {anonymized_path}/test')


for i in X_test_control:
    os.system(f'mv {i} {control_path}/test')
