...
save_path_all = Path('../data/frcnn_data')
data_path = Path('../data')
im_save_path = Path('')
...
def process_annotated_nifty(pt_path, split, image_name='final_cropped_128_pet.nii.gz', width=512, height=512):
    images_ax = []
    images_cor = []
    images_sag = []
    ...
    image_pet = tio.ScalarImage(pt_path / image_name)
    transform = tio.Resize((512,512,512))
    image_pet_trans = transform(image_pet)
    img_array_pet = image_pet_trans.data
    image = np.array(img_array_pet[0]).astype(np.float32)
    ...
    for i in df_ann.iloc():
        ...
        for k in range(voxel_offset_diameter):
            ...
            axial_img = np.stack((image[:,:,slice_idx[2]],)*channels, axis=-1)
            ...
            #Coronal View
            coronal_img = np.stack((image[:,slice_idx[1],:],)*channels, axis=-1)
            ...
            #Sagital View
            sagtial_img = np.stack((image[slice_idx[0],:,:],)*channels, axis=-1)                
...
def process_control_nifty(pt_path, split, slice_per_axis, image_name='final_cropped_128_pet.nii.gz', width=512, height=512):
    images_ax = []
    images_cor = []
    images_sag = []
    global img_id
    ...
    for i in indexes:
        ...
        #Axial View
        axial_img = np.stack((image[:,:,i],)*channels,axis=-1)
        
        
        #max for png normalization
        #max_val = np.amax(axial_img)
        ...
        coronal_img = np.stack((image[:,i,:],)*channels, axis=-1)
        save_path = str(save_path_all / 'coronal'/f'{split}' / f"3channel_anonymized_coronal_id_{img_id}")
        np.save(save_path, coronal_img)
        ...
        sagtial_img = np.stack((image[i,:,:],)*channels,axis=-1)    
        save_path = str(save_path_all / 'sagital'/f'{split}'/ f"3channel_anonymized_sagital_id_{img_id}")
        np.save(save_path, sagtial_img)
